...
if (!fs.existsSync(tfFilePath)) {
  throw new Error("Terraform state file does not exist");
}
...
(async () => {
  console.log("Deploy in 3 seconds...");
  ...
  if (fs.existsSync(lockFilePath)) {
    console.error("Lockfile found! Terminating ...");
    process.exit();
  }
  ...
  fs.writeFileSync(
    lockFilePath,
    "Stop node from running concurrently with itseft"
  );
  ...
  await exec(
    `zip -r ${zipPath} . -x terraform/\\* -x node_modules/\\* -x \\*/node_modules/\\* -x \\*/dist/\\* -x \\*/.cache/\\* -x .git/\\* -x \\*.DS_Store`,
    {
      cwd: rootDir,
  ...
  console.log("Remove deployment file...");
